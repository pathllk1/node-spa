...
export async function fetchCurrentUserFirmName(state) {
    try {
        ...
        if (data.success && data.data?.name) {
            ...
            if (window.currentUserFirmName !== data.data.name) {
    ...
    try {
        ...
        if (!stockResponse.ok) {
            ...
            if (stockData.success) {
                state.stocks = Array.isArray(stockData.data) ? stockData.data : [];
            } else {
                console.warn('Failed to fetch stocks:', stockData.error);
                state.stocks = [];
            }
        }
        ...
        try {
            ...
            if (!partyResponse.ok) {
                ...
                if (partyData.success) {
                    state.parties = Array.isArray(partyData.data) ? partyData.data : [];
                } else {
                    console.warn('Failed to fetch parties:', partyData.error);
                    state.parties = [];
                }
            }
        } catch (e) {
        ...
        if (!state.meta.billNo || state.meta.billNo === 'Will be generated on save') {
            ...
            try {
                ...
                if (!previewResponse.ok) {
                    console.warn('Failed to fetch bill number preview:', previewResponse.status);
                } else {
