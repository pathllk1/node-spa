...
/**
 * Builds and inserts all ledger entries for a PURCHASE bill.
 *
 * Purchase double-entry:
 *   Purchase A/c           DR  (expense  — taxable value of goods)
 *   Other Charges A/c      DR  (expense  — freight, packing, etc.)
 *   CGST Input Credit A/c  DR  (asset    — recoverable input tax)
 *   SGST Input Credit A/c  DR  (asset    — recoverable input tax)
 *   IGST Input Credit A/c  DR  (asset    — recoverable input tax)
 *   Round Off              DR/CR as needed
 *   Supplier / Party A/c   CR  (creditor — total payable to supplier)
 */
async function postPurchaseLedger({ firmId, billId, voucherId, billNo, billDate, party, ntot, cgst, sgst, igst,
  ...
  // 1. Supplier / Party CR  (creditor — we owe them ntot)
  docs.push({
    ...base,
  ...
  if (cgst > 0) docs.push({ ...base, account_head: 'CGST Input Credit', account_type: 'ASSET',
    debit_amount: cgst, credit_amount: 0, narration: `CGST Input on Purchase Bill No: ${billNo}`,
    party_id: null, stock_id: null, stock_reg_id: null });
  if (sgst > 0) docs.push({ ...base, account_head: 'SGST Input Credit', account_type: 'ASSET',
    debit_amount: sgst, credit_amount: 0, narration: `SGST Input on Purchase Bill No: ${billNo}`,
    party_id: null, stock_id: null, stock_reg_id: null });
  if (igst > 0) docs.push({ ...base, account_head: 'IGST Input Credit', account_type: 'ASSET',
    debit_amount: igst, credit_amount: 0, narration: `IGST Input on Purchase Bill No: ${billNo}`,
    party_id: null, stock_id: null, stock_reg_id: null });
  ...
  if (Math.abs(parseFloat(rof)) > 0) {
    ...
    docs.push({ ...base, account_head: 'Round Off', account_type: 'EXPENSE',
      ...
      narration: `Round Off on Purchase Bill No: ${billNo}`,
      party_id: null, stock_id: null, stock_reg_id: null });
  }
  ...
  if (otherCharges?.length > 0) {
    for (const charge of otherCharges) {
      ...
      if (chargeAmount > 0) {
        docs.push({ ...base,
          account_head: charge.type || 'Other Charges', account_type: 'EXPENSE',
          debit_amount: chargeAmount, credit_amount: 0,
          narration: `${charge.type} on Purchase Bill No: ${billNo}`,
          party_id: null, stock_id: null, stock_reg_id: null });
      }
    }
  }

  // 5. Purchase account DR  (expense — taxable value of items only)
  const taxableItemsTotal = cart.reduce(
    (sum, item) => sum + (item.qty * item.rate * (1 - (item.disc || 0) / 100)), 0
  );
  docs.push({ ...base, account_head: 'Purchase', account_type: 'EXPENSE',
    debit_amount: taxableItemsTotal, credit_amount: 0,
    narration: `Purchase Bill No: ${billNo}`,
    party_id: null, stock_id: null, stock_reg_id: null });
...
export const createBill = async (req, res) => {
  ...
  try {
    // Generate purchase bill number atomically — $inc ensures no duplicate under concurrency
    ...
    // A. Insert Purchase Bill Header
    // supply = supplier name (who we bought from); firm = our firm (buyer)
    const [newBill] = await Bill.create([{
      firm_id:           firmId,
      voucher_id:        voucherId,
      bno:               billNo,
      bdate:             meta.billDate,
      supply:            partyDoc.firm      || '',    // supplier name
      firm:              firmName,                    // our firm (buyer)
      addr:              partyDoc.addr      || '',
    ...
    // B. Process cart items — ADD stock (purchase increases inventory)
    const stockRegDocs = [];
    for (const item of cart) {
      ...
      // Resolve batch — create a new entry if this batch hasn't been received before
      let batchIndex = -1;
      ...
      if (batchIndex !== -1) {
        // Existing batch — add quantity, update purchase price
        ...
        // New batch received in this purchase
        batches.push({
      ...
      const newTotalQty = batches.reduce((s, b) => s + (parseFloat(b.qty) || 0), 0);
      const newRate     = parseFloat(item.rate) || stockRecord.rate;
      ...
      await Stock.findOneAndUpdate(
        { _id: item.stockId, firm_id: firmId },
        {
          $set: {
            qty:    newTotalQty,
            batches,
            rate:   newRate,           // update cost price to latest purchase rate
            total:  newTotalQty * newRate,
            user:   actorUsername,
          },
        },
        { session }
      ...
      stockRegDocs.push({
        ...
        bill_id:        billId,
        user:           actorUsername,
        qtyh:           newTotalQty,   // qty on hand after this purchase
      });
    }

    await StockReg.insertMany(stockRegDocs, { session });

    // C. Post purchase ledger entries (within the same session)
    await postPurchaseLedger({ firmId, billId, voucherId, billNo, billDate: meta.billDate, party: partyDoc, ntot, cgst, sgst, igst, rof, otherCharges, cart, actorUsername, session });
    ...
    if (!res.headersSent) {
      res.status(500).json({
        success: false,
        error:   err.message || 'Failed to create purchase bill',
...
export const updateBill = async (req, res) => {
  ...
  try {
    ...
    // Step 1: Reverse old purchase — subtract qty that was added by the original purchase
    ...
    for (const existingItem of existingItems) {
      ...
      if (batchIndex !== -1) {
        // Subtract the previously purchased qty (clamp to 0 to avoid negative stock)
        batches[batchIndex].qty = Math.max(0, batches[batchIndex].qty - existingItem.qty);
      }
      // If the batch no longer exists it was already removed — skip silently
    ...
    await Bill.findOneAndUpdate(
      { _id: billId, firm_id: firmId },
      {
        $set: {
          bdate:             meta.billDate,
          supply:            party.firm      || '',   // supplier name
          firm:              firmName,                // our firm (buyer)
          addr:              party.addr      || '',
    ...
    // Step 5: Process new cart items — ADD stock (purchase)
    const stockRegDocs = [];
    for (const item of cart) {
      const lineTotal = item.qty * item.rate * (1 - (item.disc || 0) / 100);
    ...
    // Step 6: Delete old purchase ledger entries and re-post
    await Ledger.deleteMany({ voucher_id: existingBill.voucher_id, voucher_type: 'PURCHASE', firm_id: firmId }, { session });
    await postPurchaseLedger({ firmId, billId, voucherId: existingBill.voucher_id, billNo: existingBill.bno, billDate: meta.billDate,
...
export const cancelBill = async (req, res) => {
  try {
    ...
    // Reverse purchase — subtract qty that was added by the original purchase
    ...
    for (const item of items) {
      ...
      if (batchIndex !== -1) {
        // Subtract previously purchased qty (clamp to 0 to avoid negative stock)
        batches[batchIndex].qty = Math.max(0, batches[batchIndex].qty - item.qty);
      }
      // If batch not found, stock was already manually adjusted — skip silently
    ...
    // Delete purchase ledger entries
    await Ledger.deleteMany({ voucher_id: bill.voucher_id, voucher_type: 'PURCHASE', firm_id: firmId });
...
export const getPartyBalance = async (req, res) => {
  try {
    ...
    const [result] = await Ledger.aggregate([
      {
        $match: {
          ...
          account_type: 'CREDITOR',          // purchase: supplier is a creditor
        },
      },
      {
        $group: {
    ...
    // For purchases: positive balance = amount still payable to supplier
