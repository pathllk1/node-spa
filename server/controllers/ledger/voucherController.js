...
export const createVoucher = async (req, res) => {
    ...
    try {
        ...
        const partyStmt = db.prepare('SELECT firm FROM parties WHERE id = ?');
        const partyData = partyStmt.get(parseInt(party_id));
...
export const getVouchers = async (req, res) => {
    try {
        ...
        let query = `SELECT DISTINCT voucher_no, voucher_id, voucher_type, transaction_date, narration, party_id,
                     SUM(CASE WHEN debit_amount > 0 THEN debit_amount ELSE 0 END) as amount
                     FROM ledger
...
export const getVoucherById = async (req, res) => {
    try {
        ...
        const stmt = db.prepare('SELECT * FROM ledger WHERE voucher_id = ? AND firm_id = ? AND voucher_type IN ("PAYMENT", "RECEIPT") LIMIT 1');
        const voucher = stmt.get(voucherId, firmId);
