...
/**
 * Prisma seed script to create initial firms and admin users
...
async function seed() {
  ...
  try {
    ...
    let firm1 = await prisma.firm.findUnique({
      where: { code: 'ACME' }
    });
    ...
    let firm2 = await prisma.firm.findUnique({
      where: { code: 'BUILD' }
    });

    if (!firm2) {
      firm2 = await prisma.firm.create({
        data: {
    ...
    let firm3 = await prisma.firm.findUnique({
      where: { code: 'METRO' }
    });

    if (!firm3) {
      firm3 = await prisma.firm.create({
        data: {
